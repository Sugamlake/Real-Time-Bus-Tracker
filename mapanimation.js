// This array contains the coordinates for all bus stops between MIT and Harvard
const busStops = [
  [-71.093729, 42.359244],
  [-71.094915, 42.360175],
  [-71.0958, 42.360698],
  [-71.099558, 42.362953],
  [-71.103476, 42.365248],
  [-71.106067, 42.366806],
  [-71.108717, 42.368355],
  [-71.110799, 42.369192],
  [-71.113095, 42.370218],
  [-71.115476, 42.372085],
  [-71.117585, 42.373016],
  [-71.118625, 42.374863],
];

// TODO: add your own access token
mapboxgl.accessToken = 'pk.eyJ1IjoiZWRpZGllcjYxNSIsImEiOiJja3k3Z3N4dDAxNWZ3Mm9tbmRwYjMzbzIxIn0.SaAP9c92ED1dYqfY0_0gig';

// This is the map instance
let map = new mapboxgl.Map({
  container: 'map',
  style: 'mapbox://styles/mapbox/streets-v11',
  center: [-71.104081, 42.365554],
  zoom: 14,
});

let element = document.createElement('div');
element.className = 'marker';

// TODO: add a marker to the map at the first coordinates in the array busStops. The marker variable should be named "marker"
let marker = new mapboxgl.Marker(element)
  .setLngLat(busStops[0])
  .addTo(map);
// counter here represents the index of the current bus stop
let counter = 0;
function move() {
  // TODO: move the marker on the map every 1000ms. Use the function marker.setLngLat() to update the marker coordinates
  // Use counter to access bus stops in the array busStops
  // Make sure you call move() after you increment the counter.
  if (counter <= busStops.length)
  {
    let marker1 = new mapboxgl.Marker()
      .setLngLat(busStops[counter])
      .addTo(map);
    marker.setLngLat(busStops[counter]).addTo(map);
    setTimeout(move,1000);
    counter++;
    //return move();
  }
}

// Obtenga la distancia en metros mediante el método computeDistanceBetween
// Desde la extensión de Google Maps usando coordenadas simples
var distanceInMeters = google.maps.geometry.spherical.computeDistanceBetween(
    new google.maps.LatLng({
        lat: busStops[0][1], 
        lng: busStops[0][0]
    }),
    new google.maps.LatLng({
        lat: busStops[11][1], 
        lng: busStops[11][0]
    })
);

// Imprime: Distance in Meters:  286562.7470149898
console.log("Distancia en metros:", distanceInMeters);
document.getElementById("distance").innerHTML="<b>Distance in Meters: "+distanceInMeters+"</b><br><b>Distance in Kilometers: "+distanceInMeters*0.001+"</b>";

// Imprime: Distance in Kilometers:  286.5627470149898
console.log("Distancia en kilómetros:", (distanceInMeters * 0.001));

// Do not edit code past this point
if (typeof module !== 'undefined') {
  module.exports = { move };
}